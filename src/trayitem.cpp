...
TrayItem::TrayItem(Window window, const TrayItemArgs args) {
    ...
    m_MultiWindowRestore = readSetting(NOARG, "_MultiWindowRestore", DEFAULT_MultiWindowRestore);
...
void TrayItem::restoreWindow() {
    ...
    if (m_iconified) {
        ...
        if (m_MultiWindowRestore) {    // derived from the config file (if present) or DEFAULT_MultiWindowRestore
            /*
